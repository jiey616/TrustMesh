...
import type { AssistantMessage, AssistantResult, AssistantToolCall } from '@/types/assistant'
...
export const useAssistantStore = create<AssistantState>()((set) => ({
  ...
  addToolCall: (messageId, tool, args) =>
    set((s) => ({
      messages: s.messages.map((m) =>
        m.id === messageId
          ? { ...m, toolCalls: [...(m.toolCalls ?? []), { tool, args, status: 'running' as const }] }
          : m
      ),
    })),
        ...
        m.id === messageId
          ? {
              ...
              toolCalls: m.toolCalls?.map((tc: AssistantToolCall) =>
                tc.tool === tool && tc.status === 'running' ? { ...tc, status: 'done' as const } : tc
              ),
            }
          : m
      ),
  ...
  addToolResult: (messageId, result) =>
    set((s) => ({
      messages: s.messages.map((m) =>
        m.id === messageId ? { ...m, results: [...(m.results ?? []), result] } : m
      ),
    })),
