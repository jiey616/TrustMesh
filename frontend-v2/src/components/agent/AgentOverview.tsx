      ...
      <Row gutter={[16, 16]}>
        <Col xs={12} lg={6}>
          <Card bordered={false} className="!bg-[color:var(--surface)]">
            ...
            <div className="text-2xl font-bold text-[color:var(--text-primary)]">{stats.projects_managed}</div>
          </Card>
        </Col>
        <Col xs={12} lg={6}>
          <Card bordered={false} className="!bg-[color:var(--surface)]">
            ...
            <div className="text-2xl font-bold text-[color:var(--text-primary)]">{stats.tasks_created}</div>
        ...
        <Col xs={12} lg={6}>
          <Card bordered={false} className="!bg-[color:var(--surface)]">
            ...
            <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">{stats.tasks_done} 完成 / {stats.tasks_failed} 失败</div>
          </Card>
        </Col>
        <Col xs={12} lg={6}>
          <Card bordered={false} className="!bg-[color:var(--surface)]">
            ...
            <div className="text-2xl font-bold text-[color:var(--text-primary)]">{stats.planning_replies}</div>
          </Card>
        </Col>
      </Row>
    )
    ...
    <Row gutter={[16, 16]}>
      <Col xs={12} lg={6}>
        <Card bordered={false} className="!bg-[color:var(--surface)]">
          ...
          <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">{stats.todos_done} 完成 / {stats.todos_failed} 失败</div>
        </Card>
      </Col>
      <Col xs={12} lg={6}>
        <Card bordered={false} className="!bg-[color:var(--surface)]">
          ...
          <div className="text-2xl font-bold text-[color:var(--text-primary)]">{stats.todos_total}</div>
          <div className="mt-1 flex gap-1.5 flex-wrap">
      ...
      <Col xs={12} lg={6}>
        <Card bordered={false} className="!bg-[color:var(--surface)]">
          ...
          <div className="text-2xl font-bold text-[color:var(--text-primary)]">{formatDuration(stats.avg_response_time_ms)}</div>
          <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">接收到开始执行</div>
        </Card>
      </Col>
      <Col xs={12} lg={6}>
        <Card bordered={false} className="!bg-[color:var(--surface)]">
          ...
          <div className="text-2xl font-bold text-[color:var(--text-primary)]">{formatDuration(stats.avg_completion_time_ms)}</div>
          <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">开始到完成</div>
        </Card>
      </Col>
    </Row>
  )
...
function AgentDailyChart({ stats }: { stats: AgentStats }) {
  ...
  if (!hasActivity || data.length === 0) {
    return (
      <Card bordered={false} className="!bg-[color:var(--surface)]">
        <div className="mb-2 text-base font-medium text-[color:var(--text-primary)]">
          {isPM ? '每日任务动态（近 30 天）' : '每日工作量（近 30 天）'}
        </div>
        <div className="flex h-[220px] items-center justify-center text-sm text-[color:var(--text-tertiary)]">暂无活动数据</div>
      </Card>
    )
  }
  ...
  return (
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      ...
      <div className="mt-1 flex justify-between text-[10px] text-[color:var(--text-quaternary)]">
...
function AgentWorkload({ stats }: { stats: AgentStats }) {
  ...
  return (
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      <div className="mb-2 flex items-center gap-2 text-base font-medium text-[color:var(--text-primary)]">
        {isPM ? '进行中的任务' : '当前工作负载'}
        ...
        {stats.current_workload.map((item) => (
          <div key={item.todo_id || item.task_id} className="flex items-center justify-between gap-3 py-2.5 first:pt-0 last:pb-0">
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm text-[color:var(--text-primary)]">{isPM ? item.task_title : item.todo_title}</div>
              {!isPM && item.task_title && (
                <div className="mt-0.5 truncate text-xs text-[color:var(--text-tertiary)]">{item.task_title}</div>
              )}
            </div>
            <div className="shrink-0 text-xs text-[color:var(--text-quaternary)]">{dayjs(item.started_at).fromNow()}</div>
          </div>
        ))}
      </div>
    </Card>
...
function EmptyInsightCard({ title, description }: { title: string; description: string }) {
  return (
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      <div className="mb-2 text-base font-medium text-[color:var(--text-primary)]">{title}</div>
      <div className="flex h-[140px] items-center justify-center text-center text-sm text-[color:var(--text-tertiary)]">{description}</div>
    </Card>
  )
}
  ...
  return (
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      ...
      {aging.some((i) => i.count > 0) ? (
          ...
          {aging.map((i) => (
            <div key={i.label} className="space-y-1.5">
              <div className="flex items-center justify-between text-sm text-[color:var(--text-primary)]">
                <span>{i.label}</span>
                <span className="text-[color:var(--text-tertiary)]">{i.count}</span>
              </div>
        ...
        <div className="flex h-[140px] items-center justify-center text-sm text-[color:var(--text-tertiary)]">当前没有积压项</div>
      )}
    </Card>
  )
}
      ...
      <div className="space-y-3">
        {priorityBreakdown.map((i) => (
          <div key={i.priority} className="space-y-1.5">
            <div className="flex items-center justify-between text-sm text-[color:var(--text-primary)]">
              <span>{i.label}</span>
              <span className="text-[color:var(--text-tertiary)]">{i.done} 完成 / {i.failed} 失败</span>
            </div>
            <Progress percent={Math.round((i.total / max) * 100)} showInfo={false} strokeColor="#22d3ee" />
            <div className="flex items-center justify-between text-xs text-[color:var(--text-quaternary)]">
        ...
        {projectContribution.map((i) => (
          <div key={i.project_id} className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="truncate text-sm text-[color:var(--text-primary)]">{i.project_name}</div>
              <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">{i.total} 项 · {i.done} 完成 · {i.failed} 失败</div>
            </div>
            <div className="shrink-0 text-right">
              <div className="text-sm font-medium text-[color:var(--text-primary)]">{formatPercent(i.completion_rate ?? 0)}</div>
              <div className="text-xs text-[color:var(--text-tertiary)]">{i.pending} 待处理 / {i.in_progress} 执行中</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-[color:var(--line)] p-3">
          <div className="text-xs text-[color:var(--text-tertiary)]">最老待处理</div>
          <div className="mt-1 text-lg font-semibold text-[color:var(--text-primary)]">{formatDurationShort(insights.oldest_pending_ms)}</div>
        </div>
        <div className="rounded-lg border border-[color:var(--line)] p-3">
          <div className="text-xs text-[color:var(--text-tertiary)]">最长执行中</div>
          <div className="mt-1 text-lg font-semibold text-[color:var(--text-primary)]">{formatDurationShort(insights.longest_in_progress_ms)}</div>
        </div>
        <div className="rounded-lg border border-[color:var(--line)] p-3">
          <div className="text-xs text-[color:var(--text-tertiary)]">24 小时未闭环</div>
          <div className="mt-1 text-lg font-semibold text-[color:var(--text-primary)]">{insights.pending_over_24h}</div>
        </div>
        <div className="rounded-lg border border-[color:var(--line)] p-3">
          <div className="text-xs text-[color:var(--text-tertiary)]">近 7 天失败</div>
          <div className="mt-1 text-lg font-semibold text-[color:var(--text-primary)]">{insights.failures_last_7d}</div>
        </div>
      </div>
      {!isPM && (insights.response_p90_ms !== null || insights.completion_p90_ms !== null) && (
        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
          <div className="rounded-lg bg-[color:var(--surface)] p-3">
            <div className="text-xs text-[color:var(--text-tertiary)]">P90 响应时长</div>
            <div className="mt-1 font-medium text-[color:var(--text-primary)]">{formatDuration(insights.response_p90_ms)}</div>
          </div>
          <div className="rounded-lg bg-[color:var(--surface)] p-3">
            <div className="text-xs text-[color:var(--text-tertiary)]">P90 完成时长</div>
            <div className="mt-1 font-medium text-[color:var(--text-primary)]">{formatDuration(insights.completion_p90_ms)}</div>
          </div>
        </div>
      )}
        ...
        <div className="mt-3 space-y-2">
          {(insights.risk_items ?? []).map((item) => (
            <div key={item.id} className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="truncate text-sm text-[color:var(--text-primary)]">{item.title}</div>
                <div className="mt-0.5 truncate text-xs text-[color:var(--text-tertiary)]">{item.project_name} · {item.subtitle}</div>
              </div>
              <div className="shrink-0 text-right">
                <Tag className="!text-[10px]" color={item.status === 'in_progress' ? 'blue' : 'default'}>
                  {item.status === 'in_progress' ? '执行中' : '待处理'}
                </Tag>
                <div className="mt-0.5 text-xs text-[color:var(--text-quaternary)]">{formatDurationShort(item.age_ms)}</div>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="mt-3 text-sm text-[color:var(--text-tertiary)]">当前没有需要关注的{isPM ? '任务' : 'Todo'}风险项。</div>
      )}
    </Card>
  )
}
...
function InsightsSummaryCard({ insights }: { insights: AgentInsights }) {
  ...
  return (
    <Card bordered={false} className="!bg-[color:var(--surface)]">
      <div className="mb-2 text-base font-medium text-[color:var(--text-primary)]">Insights</div>
      {insights.summary && <p className="mb-3 text-sm text-[color:var(--text-primary)]">{insights.summary}</p>}
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12}>
