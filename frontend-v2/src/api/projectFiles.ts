...
export async function uploadProjectFile(projectId: string, formData: FormData) {
  return apiClient.post(`/api/v1/projects/${projectId}/files`, { body: formData }).json<ApiResponse<ProjectFile>>()
}

export async function listProjectFiles(projectId: string) {
  return apiClient.get(`/api/v1/projects/${projectId}/files`).json<ApiListResponse<ProjectFile>>()
}

/** 按目录浏览文件（根目录 parent_id 传空字符串） */
export async function browseProjectFiles(projectId: string, parentId: string) {
  return apiClient
    .get(`/api/v1/projects/${projectId}/files/browse`, { searchParams: { parent_id: parentId } })
...
export async function createProjectFolder(projectId: string, input: CreateProjectFolderRequest) {
  return apiClient.post(`/api/v1/projects/${projectId}/folders`, { json: input }).json<ApiResponse<ProjectFile>>()
}

export async function renameProjectFile(projectId: string, fileId: string, name: string) {
  return apiClient.patch(`/api/v1/projects/${projectId}/files/${fileId}/rename`, { json: { name } }).json<ApiResponse<ProjectFile>>()
}

export async function deleteProjectFile(projectId: string, fileId: string) {
  return apiClient.delete(`/api/v1/projects/${projectId}/files/${fileId}`).json<ApiResponse<ProjectFile>>()
}

export async function downloadProjectFile(projectId: string, fileId: string): Promise<Blob> {
  const res = await apiClient.get(`/api/v1/projects/${projectId}/files/${fileId}/content`)
  return res.blob()
}

export async function batchDeleteProjectFiles(projectId: string, fileIds: string[]) {
  return apiClient
    .post(`/api/v1/projects/${projectId}/files/batch-delete`, { json: { file_ids: fileIds } })
