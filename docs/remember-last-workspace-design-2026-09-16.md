...
2. **首屏额外一轮请求**：校准前会先发一轮**无 org 头**的数据请求（对该用户安全，后端退化为 user 维度），校准后再 `removeQueries` 重取一轮。这与「新鲜登录」现状一致，但相比「今天的刷新（因持久化 personalOrgId 命中同值不发）」会**多一轮**。是否接受？（推荐接受；若要消除需门控所有受保护查询，属更大改动。）
