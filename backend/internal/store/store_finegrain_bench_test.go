...
//
// ⚠️ 2026-09-16 T2.5 回退说明：本文件原另有 AC-1 / AC-3 两条**断言型**用例
// （TestSingleAggregatePersistDoesNotBlockOtherAggregates / TestCancelTaskEarlyReturnReleasesAggregateLocks），
// 二者断言的是「四域调用点已迁到聚合锁」这一已回退的效果：回退后它们既不依赖真实
// 调用点、又会恒绿，属**无鉴别力的假绿护栏**，故随调用点一并移除。
// 保留的 Benchmark* 只度量锁原语自身的并发度（不依赖任何调用点迁移），继续作为
// T05 原子性整批迁移的基线数据。
//
