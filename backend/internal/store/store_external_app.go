...
func (s *Store) CreateExternalApp(sc Scope, in CreateExternalAppInput) (*model.ExternalAppView, string, *transport.AppError) {
	...
	// 阶段 1 双写漏网：原本恒挂作者个人租户，企业租户下注册的应用带租户头
	// 对同租户成员不可见（与 2-2 / 2-4 / 2-5a / 2-5b 同类）。
	app.OrgID = s.resolveOwnerOrgUnsafe(sc)
...
// canAccessExternalApp reports whether the scope may see (and launch) the app.
// Public apps are visible to everyone — that is the product semantics of a
// platform-level mount, not a leak. Private apps: creator + same-org members.
func canAccessExternalApp(sc Scope, a *model.ExternalApp) bool {
	if a == nil {
		return false
	}
	if sc.System {
	...
	return visibleToScope(sc, a.OrgID, a.CreatedBy)
}
...
func (s *Store) GetExternalApp(sc Scope, id string) (*model.ExternalAppView, *transport.AppError) {
	...
	if !ok || !canAccessExternalApp(sc, a) {
...
// may update it.
func (s *Store) UpdateExternalApp(sc Scope, id string, in UpdateExternalAppInput) (*model.ExternalAppView, *transport.AppError) {
	s.mu.Lock()
	defer s.mu.Unlock()
	a, ok := s.externalApps[id]
	if !ok {
	...
	// 管理操作仍限创建者本人：同租户成员可以看见/启动，但不能改别人的应用。
	if !sc.System && a.CreatedBy != sc.UserID {
...
func (s *Store) DeleteExternalApp(sc Scope, id string) *transport.AppError {
	s.mu.Lock()
	defer s.mu.Unlock()
	a, ok := s.externalApps[id]
	if !ok {
